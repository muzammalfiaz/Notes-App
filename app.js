...
function getSentNotes() { try { return JSON.parse(localStorage.getItem(SENT_KEY) || "[]"); } catch { return []; } }
function saveSentNotes(items) { localStorage.setItem(SENT_KEY, JSON.stringify(items)); }
function pruneSentNotes() {
  const cutoff = Date.now() - TWO_DAYS;
  saveSentNotes(getSentNotes().filter(n => n && (n.created_at || 0) >= cutoff));
}
function trackSentNote(noteId) {
  pruneSentNotes();
  const sent = getSentNotes().filter(n => n.id !== noteId);
  sent.unshift({ id:noteId, created_at:Date.now(), seen:false });
  saveSentNotes(sent);
}
function markSentSeen(noteId) {
  saveSentNotes(getSentNotes().map(n => n.id === noteId ? { ...n, seen:true } : n));
}
